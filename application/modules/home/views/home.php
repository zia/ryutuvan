...
<table id=mother_table>
	...
	<tr>
		...
		*/
		...
		?>
		<td class=SO_td4>
		<input class=SO_input2 type=text pattern=[0-9].{0,} name=r<?=$r?>c<?=$c?> data-id=<?=$product->id?> title=<?=$infos[$p]->data > 9999 ? $infos[$p]->data:'&nbsp;'?> value=<?=$infos[$p++]->data?> id=r<?=$r?>c<?=$c++?> maxlength=8 minlength=1>
		</td>
		<?php
		}
		}
